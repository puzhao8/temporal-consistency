...
if __name__ == "__main__":
    ...
    data_folder = Path("E:\PyProjects/temporal-consistency\data\Ljusdals")

    fireEvent = os.path.split(data_folder)[-1]

    data_name = 's1_data_kmap'
    data_dir = data_folder / f"{data_name}"
    ...
    np.save(f"data/{fireEvent}_{data_name}_{n}x{l}x{c}x{w}x{w}_20m_iter{max_iter}.npy", samples)
